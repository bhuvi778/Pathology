...
import { formatDate, calculateFlag } from '../../utils/helpers';
...
export default function FillReportReading() {
  ...
  const saveReport = async (report, idx, action = 'save') => {
    setSaving(idx);
    try {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Fill Report Reading</h1>
          {appointment && (
      ...
      {reports.length === 0 ? (
        <div className="card text-center py-12">
          <p className="text-slate-400 text-lg">No reports found for this appointment</p>
        </div>
      ) : (
          ...
          <div key={report._id} className="card">
            ...
            {report.status === 'verified' && (
              <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg flex items-center gap-2">
                ...
                <span className="text-sm text-green-700 font-medium">This report has been verified and signed</span>
              </div>
            )}
          </div>
        ))
      ...
      <div className="hidden">
        <div ref={printRef}>
          {printReport && <ReportPrint report={printReport} appointment={appointment} />}
        </div>
      </div>
    </div>
  );
